module.exports = async ( fs, debug ) => {
    try {
        ...
        //MOD TABLES
        tables.modTable = tableList.find(tl => tl.id === 'crew_rating_per_mod_rarity_level_tier').rowList.reduce((obj, item) => {
        ...
        tables.completedGearTable = [];
        tables.gearTable.reduce((gpAtTier, item) => {
            gpAtTier += (item * 6);
            tables.completedGearTable.push(gpAtTier)
